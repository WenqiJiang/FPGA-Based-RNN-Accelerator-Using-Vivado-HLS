...
int main(int argc, char *argv[]) {
  ...
  // RNN
  FDATA_T* rnn_last_state = 
      (FDATA_T*) MALLOC(sizeof(FDATA_T) * RNN_BATCH_SIZE * RNN_STATE_SIZE);
  FDATA_T* rnn_input_state = 
      (FDATA_T*) MALLOC(sizeof(FDATA_T) * RNN_BATCH_SIZE * RNN_INPUT_SIZE);
  FDATA_T* rnn_bias = 
  ...
  FDATA_T* softmax_result = 
      (FDATA_T*) MALLOC(sizeof(FDATA_T) * SM_BATCH_SIZE * SM_CLASS_SIZE);
  IDATA_T* argmax_result = 
      (IDATA_T*) MALLOC(sizeof(IDATA_T) * SM_BATCH_SIZE);
...
// transpose kernels                     <
  ...
  for (LDATA_T compute_time = 0; compute_time < SAMPLE_NUM / FC_BATCH_SIZE; 
       compute_time++) {
    ...
    for (LDATA_T i = 0; i < SAMPLE_LEN; i++) {
      for (LDATA_T j = 0; j < RNN_BATCH_SIZE; j++) {
        LDATA_T sample_index = compute_time * SAMPLE_LEN * RNN_BATCH_SIZE + 
                               j * SAMPLE_LEN + i;
        ...
        copy_data<FDATA_T, LDATA_T>(&word_embedding[word_embedding_index], 
                                    &rnn_input_state[rnn_input_state_index], 
                                    RNN_INPUT_SIZE);
      }
#ifdef DEBUG
...
#ifdef DEBUG
      print_data<FDATA_T, LDATA_T>(rnn_output_state, 
                                   RNN_STATE_SIZE * RNN_BATCH_SIZE);
#endif
      // NOT SURE, can sds_alloc use this pointer exchange operation?
    ...
    for (int i = compute_time * RNN_BATCH_SIZE; 
         i < (compute_time + 1) * RNN_BATCH_SIZE; i++) {
