...
void fc_compute(FDATA_T** input_feature_map_reg,
  ...
  // initialization
  FDATA_T **local_reg = (FDATA_T**) malloc_2d_array(FC_TILE_BATCH, FC_INPUT_SIZE);
...
//#pragma HLS ARRAY_PARTITION variable=local_reg dim=1 cyclic factor=2
  for (LDATA_T iter = 0; iter < BATCH_SIZE / FC_TILE_BATCH; iter++) {

    LDATA_T start_batch = iter * FC_TILE_BATCH;

    for (LDATA_T batch_idx = 0; batch_idx < FC_TILE_BATCH; batch_idx++) {
...
//#pragma HLS UNROLL complete
  ...
  free_2d_array(local_reg, FC_TILE_BATCH, FC_INPUT_SIZE);
}
...
void wrapper_fc(FDATA_T input_feature_map[BATCH_SIZE * FC_INPUT_SIZE],
  ...
  FDATA_T **input_feature_map_reg = (FDATA_T**) malloc_2d_array(BATCH_SIZE, FC_INPUT_SIZE);
  FDATA_T output_feature_map_reg[BATCH_SIZE];
////////////////////       MENTION       /////////////////////
// unroll in dimension 1 should be equal to FC_TILE_BATCH 
...
///*
 ...
 // */
  ...
  free_2d_array(input_feature_map_reg, BATCH_SIZE,FC_INPUT_SIZE);
}
